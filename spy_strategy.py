...
REBAL_DROP = 0.03   # sell after 3% drop from trailing high
REBUY_REB  = 0.02   # rebuy after +2% rebound from trough
...
def main():
    ...
    buy_hold_df = pd.DataFrame({
        ...
        'price': prices.values,
        'portfolio_value': buy_hold_portfolio.values
    })
    ...
    strategy_df = pd.DataFrame({
        'timestamp': portfolio.index,
        'portfolio_value': portfolio.values,
        'holding': holding_flags.values.astype(int)  # 1 = holding, 0 = out of market
    })
    ...
    outperformance = final_strategy - final_buyhold
